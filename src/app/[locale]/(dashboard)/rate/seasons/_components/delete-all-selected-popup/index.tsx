...
import { crud_delete_many_rate_seasons } from "@/lib/curd/rate-season";
...
export default function DeleteAllSelectedRateSeasonsPopup({
  selectedIds,
  unSelectedAll,
}: Props) {
  // rooms store
  const { remove_many_seasons } = useRateSeasonsStore();
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      const res = await crud_delete_many_rate_seasons(selectedIds);
      // remove the categories from the store
      if (res) {
        remove_many_seasons(selectedIds);
      }
      ...
      setOpen(false);
      // adding a toast
      toast.success("All Selected Seasons Where Deleted Successfully");
    } catch (err) {
        ...
        <div className="w-full h-full flex flex-col gap-4 justify-between">
          <DialogHeader>
            <DialogTitle className="mb-2">
              Delete All {selectedIds.length} Selected Seasons
            </DialogTitle>
            <DialogDescription className="text-card-foreground">
              Once deleted, these seasons will be remove the posts which use
              them.
            </DialogDescription>
          </DialogHeader>
