...
import { crud_delete_many_offers } from "@/lib/curd/offers";
...
export default function DeleteAllSelectedOfferPopup({
  selectedIds,
  unSelectedAll,
}: Props) {
  // offers store
  const { remove_many_offers } = useOfferStore();
  ...
  const handleDelete = async () => {
    ...
    try {
      // delete many offers
      const res = await crud_delete_many_offers(selectedIds);
      // remove the offers from the store
      if (res) {
        remove_many_offers(selectedIds);
      }
      ...
      setOpen(false);
      // adding a toast
      toast.success("All Selected Offers Where Deleted Successfully");
    } catch (err) {
        ...
        <div className="w-full h-full flex flex-col gap-4 justify-between">
          <DialogHeader>
            <DialogTitle className="mb-2">
              Delete All {selectedIds.length} Selected Offers
            </DialogTitle>
            <DialogDescription className="text-card-foreground">
              Are you sure you want to delete all selected offers?
            </DialogDescription>
          </DialogHeader>
