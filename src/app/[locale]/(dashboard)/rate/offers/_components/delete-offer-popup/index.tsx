...
import { crud_delete_offer} from "@/lib/curd/offers";
...
export default function DeleteOfferPopup({
  id,
  open,
  setOpen,
}: Props) {
  const { remove_offer } = useOfferStore();
  ...
  const handleDelete = async () => {
    ...
    setError("");
    try {
      const res = await crud_delete_offer(id);
      ...
      if (err == 403) {
        setError("Offer can't be deleted");
      } else {
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      >
        ...
        <div className="w-full h-full flex flex-col gap-4 justify-between">
          <DialogHeader>
            <DialogTitle className="mb-2">Delete Offer</DialogTitle>
