...
import { RatePlanTableColumns } from "./columns";
import {
...
import DeleteAllSelectedRatePlanPopup from "./delete-all-selected-popup";
import RateOverviewFiltering from "../filtering";

// The rate table and filters section

interface Props {
  meta?: Record<any, any>;
}

export default function RatePlanTable({ meta }: Props) {
  // access
  const { has } = useAccess();
  // rate store hook
  const {
  ...
  const { 
    ...
    setPageRows, 
    table 
  } = useServerTable<RatePlanInterface>({
      api: {
        url: "rate",
      },
      column: RatePlanTableColumns,
      data: data,
    ...
    <>
      ...
      <PageLayoutFilteringHeader>
        ...
        <div className="flex items-center gap-2">
          {/* the permission to delete many rates */}
          {has(["rate:delete"]) ? (
          ...
          {/* Filtering */}
          <RateOverviewFiltering filters={filters} setFilters={setFilters} />
