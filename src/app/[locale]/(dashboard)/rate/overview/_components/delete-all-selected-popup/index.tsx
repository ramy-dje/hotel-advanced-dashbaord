...
import { crud_delete_many_rates } from "@/lib/curd/rate";
...
export default function DeleteAllSelectedRatePlanPopup({
  selectedIds,
  unSelectedAll,
}: Props) {
  // rooms store
  const { remove_many_rates } = useRatePlanStore();
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      const res = await crud_delete_many_rates(selectedIds);
      // remove the categories from the store
      if (res) {
        remove_many_rates(selectedIds);
      }
      ...
      setOpen(false);
      // adding a toast
      toast.success("All Selected Rates Where Deleted Successfully");
    } catch (err) {
        ...
        <div className="w-full h-full flex flex-col gap-4 justify-between">
          <DialogHeader>
            <DialogTitle className="mb-2">
              Delete All {selectedIds.length} Selected Rates
            </DialogTitle>
            <DialogDescription className="text-card-foreground">
              Are you sure you want to delete all selected rates?
            </DialogDescription>
          </DialogHeader>
