...
import { RatePlanCategories_TableColumns } from "./columns";
import {
...
import DeleteAllSelectedRatePlanCategoriesPopup from "./delete-all-selected-popup";

// The rate categories table and filters section

interface Props {
  meta?: Record<any, any>;
}

export default function RatePlanCategoriesTable({ meta }: Props) {
  // access
  const { has } = useAccess();
  // rate categories store hook
  const {
    categories_list: data,
    categories_number: total,
    set_many,
    set_total,
  } = useRatePlanCategoriesStore();

  const [searchState, setSearchState] = useState("");

  const { isFetching, totalPages, pageRows, pagination, setPageRows, table } =
    useServerTable<RatePlanCategoryInterface>({
      api: {
        url: "rate-category",
      },
      column: RatePlanCategories_TableColumns,
      data: data,
      ...
      <PageLayoutFilteringHeader>
        ...
        <div className="flex items-center gap-2">
          ...
          {has(["blog_category:delete"]) ? (
            <DeleteAllSelectedRatePlanCategoriesPopup
