...
import { crud_delete_rate_category } from "@/lib/curd/rate-category";
...
export default function DeleteRatePlanCategoryPopup({
  id,
  open,
  setOpen,
}: Props) {
  const { remove_category } = useRatePlanCategoriesStore();
  ...
  const handleDelete = async () => {
    ...
    setError("");
    try {
      const res = await crud_delete_rate_category(id);
      ...
      if (err == 403) {
        setError("Category is used other by rate plan and can't be deleted");
      } else {
        ...
        <div className="w-full h-full flex flex-col gap-4 justify-between">
          <DialogHeader>
            ...
            <DialogDescription className="text-accent-foreground">
              ...
              also remove the category from all associated rate plans.
            </DialogDescription>
          </DialogHeader>
