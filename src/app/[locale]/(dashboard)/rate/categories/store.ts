...
interface StoreType {
  categories_list: RatePlanCategoryInterface[];
  categories_number: number;

  // actions
  set_many: (categories: RatePlanCategoryInterface[]) => void;
  add_category: (category: RatePlanCategoryInterface) => void;
  remove_category: (id: string) => void;
  remove_many_categories: (ids: string[]) => void;
  update_category: (id: string, category: RatePlanCategoryInterface) => void;

  set_total: (num: number) => void;
}

const useRatePlanCategoriesStore = create<StoreType>((set) => ({
  categories_list: [],
  categories_number: 0,
...
export default useRatePlanCategoriesStore;
