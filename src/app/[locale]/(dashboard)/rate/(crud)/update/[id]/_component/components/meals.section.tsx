...
import { detectMealPlan } from "@/lib/utils";
...
const UpdateRate_Meals_Section = forwardRef<HTMLDivElement, Props>(
  ({ id }, ref) => {
    const {
      control,
      formState: { errors },
    ...
    const toggleSelection = (title: string) => {
      ...
      const mealPlanCode = detectMealPlan(newValue);
      setValue("mealPlanCode", mealPlanCode);
...
export default UpdateRate_Meals_Section;
