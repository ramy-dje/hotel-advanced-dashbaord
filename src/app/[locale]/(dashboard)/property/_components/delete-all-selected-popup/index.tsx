...
import { crud_multiple_delete_rooms } from "@/lib/curd/room";
...
export default function DeleteAllSelectedRoomsPopup({
  selectedIds,
  unSelectedAll,
}: Props) {
  // rooms store
  ...
  const handleDelete = async () => {
    ...
    try {
      // delete many rooms
      const res = await crud_multiple_delete_rooms(selectedIds);
      ...
      setOpen(false);
      // adding a toast
      toast.success("All Selected Rooms Where Deleted Successfully");
    } catch (err) {
        ...
        <div className="w-full h-full flex flex-col gap-4 justify-between">
          <DialogHeader>
            ...
            <DialogDescription className="text-card-foreground">
              Once deleted, these rooms will be moved to the deleted rooms list,
