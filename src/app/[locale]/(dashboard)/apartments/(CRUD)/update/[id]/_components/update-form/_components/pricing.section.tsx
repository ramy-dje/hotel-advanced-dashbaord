...
// Update Room Pricing Section

interface Props {
  id: string;
}

const UpdateRoom_Pricing_Section = forwardRef<HTMLDivElement, Props>(
  ({ id }, ref) => {
    const {
      register,
    ...
    // old price controller
    const old_price_controller = useController({
      control,
      name: "old_price",
      defaultValue: [],
    });
    ...
    // set the old prices
    useEffect(() => {
      if (old_price_controller.field.value) {
        setPricingList(old_price_controller.field.value as any);
      }
    }, [old_price_controller.field.value]);
...
export default UpdateRoom_Pricing_Section;
