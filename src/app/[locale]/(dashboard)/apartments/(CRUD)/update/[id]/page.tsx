...
import UpdateRoomFrom from "./_components/update-form";

export default function UpdateRoomPage({ params }: { params: { id: string } }) {
  // fetching state
  const [isFetchingData, setIsFetchingData] = useState(false);
  // select room
  const [oldRoom, setOldRoom] = useState<RoomInterface | null>(null);
  // needed form data
    ...
    if (!params.id) {
      ...
      (async () => {
        ...
        try {
          ...
          const room = await crud_get_room_by_id(params.id);
          ...
          if (
            ...
            setRoomFeaturesFormData({
              ...
              floors: floors.map((e) => {
                const old_space =
                  room.floors.find((f) => f.id == e.id)?.rooms_number || 0;
                return { ...e, free_space: e.free_space + old_space };
              }), // check if this room has taken this floor before so we add its range to the free space
              includes: includes,
          ...
          console.log(
            "Error: fetching the room features which're needed for the creation room ",
            err
          );
  ...
  return (
    <>
      ...
      <PageLayout>
          ...
          <PageLayoutHeaderNameAndBreadcrumbs>
            ...
            <Breadcrumb>
              <BreadcrumbList>
                ...
                <BreadcrumbItem>
                  <BreadcrumbPage>Update Room</BreadcrumbPage>
                </BreadcrumbItem>
              </BreadcrumbList>
            </Breadcrumb>
          </PageLayoutHeaderNameAndBreadcrumbs>
        ...
        {!isFetchingData && oldRoom && roomFeaturesFormData ? (
          <UpdateRoomFrom
            oldRoom={oldRoom}
            formSelectData={roomFeaturesFormData}
          />
        ) : (
