...
const UpdateRoom_Floors_Section = forwardRef<HTMLDivElement, Props>(
  ({ id, floors }, ref) => {
    const {
    ...
    // set the floors to the old floors
    useEffect(() => {
      if (old_floors) {
        setFloorsList(
          old_floors.map((n) => ({
            floor_free_space: n.floor_free_space,
            floor_id: n.id,
            name: n.name,
            floor_range: n.floor_range,
...
export default UpdateRoom_Floors_Section;
