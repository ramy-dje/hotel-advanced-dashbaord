...
const CreateRoom_PoliciesAndRestrictions_Section = forwardRef<HTMLDivElement, Props>(
  ({ id, damage_fee_options = [], deposit_fee_options = [] }, ref) => {
    ...
    const damage_fees_controller = useController({
      control,
      name: "damage_fees", // This field will now store the 'id' of the selected fee
    ...
    const deposit_fees_controller = useController({
      control,
      name: "deposit_fees", // This field will now store the 'id' of the selected fee
      defaultValue: "", // Default to empty string as it's a select field
    });
    ...
    // 6. Policies: Terms: Restriction ++++ (Textarea/Editor)
    const policies_terms_restriction_controller = useController({
      control,
      name: "policies_terms_restriction",
      defaultValue: "",
    });
    ...
    return (
      <CreationFormSection ref={ref} id={id}>
        ...
        <CreationFormSectionContent>
          {/* 1. Authorized Smoking Area (Yes/No) */}
          <ToggleInput
            name="authorized_smoking_area" // Ensure this matches your schema field name
          ...
          {/* 2. Pet Friendly (Yes/No) */}
          <ToggleInput
            name="pet_friendly" // Ensure this matches your schema field name
            label="Pet Friendly"
            disabled={disabled}
          />
          ...
          <div className="flex flex-col gap-2 col-span-2">
            ...
            />
            {errors?.damage_fees ? (
              <InlineAlert type="error">{errors.damage_fees.message}</InlineAlert>
            ) : null}
          </div>
          ...
          <div className="flex flex-col gap-2 col-span-2">
            ...
            />
            {errors?.deposit_fees ? (
              <InlineAlert type="error">{errors.deposit_fees.message}</InlineAlert>
            ) : null}
          </div>
          ...
          <div className="flex flex-col gap-2 col-span-2">
            ...
            {errors?.minimum_nights ? (
              <InlineAlert type="error">{errors.minimum_nights.message}</InlineAlert>
            ) : null}
          </div>
