...
import {
  CreateFoodTypeValidationSchemaType,
  CreateFoodTypeValidationSchema,
} from "./create-food-type.schema";
import { Label } from "@/components/ui/label";
import { Input } from "@/components/ui/input";
import InlineAlert from "@/components/ui/inline-alert";
import { crud_create_food_type } from "@/lib/curd/food-type";
import toast from "react-hot-toast";
import useFoodTypesStore from "../../store";

export default function CreateFoodTypePopup() {
  // food types store hook
  const { add_type } = useFoodTypesStore();
  ...
  const {
    ...
    reset,
    formState: { errors },
  } = useForm<CreateFoodTypeValidationSchemaType>({
    resolver: zodResolver(CreateFoodTypeValidationSchema),
  });
  const [open, setOpen] = useState(false);
  const [isLoading, setIsLoading] = useState(false);
  const [error, setError] = useState("");

  const handleCreate = async (data: CreateFoodTypeValidationSchemaType) => {
    setIsLoading(true);
    setError("");
    try {
      const res = await crud_create_food_type(data.name);
