...
import { crud_delete_many_food_dishes } from "@/lib/curd/food-dish";
...
export default function DeleteAllSelectedFoodDishesPopup({
  selectedIds,
  unSelectedAll,
}: Props) {
  // food dishes store hook
  const { remove_many_dishes } = useFoodDishesStore();
  ...
  const handleDelete = async () => {
    ...
    try {
      // delete many tag
      const res = await crud_delete_many_food_dishes(selectedIds);
      // remove the dishes from the store
      if (res) {
        remove_many_dishes(selectedIds);
      }
      ...
      setOpen(false);
      // adding a toast
      toast.success("All Selected Dishes Where Deleted Successfully");
    } catch (err) {
        ...
        <div className="w-full h-full flex flex-col gap-4 justify-between">
          <DialogHeader>
            <DialogTitle className="mb-2">
              Delete All {selectedIds.length} Selected Dishes
            </DialogTitle>
            <DialogDescription className="text-card-foreground">
              Please confirm that you want to delete these dishes. This action
              is irreversible.
            </DialogDescription>
          </DialogHeader>
