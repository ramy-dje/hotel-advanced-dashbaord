...
import { FoodIngredients_TableColumns } from "./columns";
import {
...
import DeleteAllSelectedFoodIngredientsPopup from "./delete-all-selected-popup";

// The food ingredients table and filters section

interface Props {
  meta?: Record<any, any>;
}

export default function FoodIngredientsTable({ meta }: Props) {
  // access
  const { has } = useAccess();
  // food ingredients store hook
  const {
    ingredients_list: data,
    ingredients_number: total,
    set_many,
    set_total,
  } = useFoodIngredientsStore();

  const [searchState, setSearchState] = useState("");

  const { isFetching, totalPages, pageRows, pagination, setPageRows, table } =
    useServerTable<FoodIngredientInterface>({
      api: {
        url: "food/ingredients",
      },
      column: FoodIngredients_TableColumns,
      data: data,
    ...
    <>
      ...
      <PageLayoutFilteringHeader>
        ...
        <div className="flex items-center gap-2">
          {/* the permission to delete many food ingredients */}
          {has(["food_ingredient:delete"]) ? (
            <DeleteAllSelectedFoodIngredientsPopup
