...
import { crud_delete_many_food_menus } from "@/lib/curd/food-menu";
...
export default function DeleteAllSelectedFoodMenusPopup({
  selectedIds,
  unSelectedAll,
}: Props) {
  // food menus store hook
  const { remove_many_menus } = useFoodMenusStore();
  ...
  const handleDelete = async () => {
    ...
    try {
      // delete many tag
      const res = await crud_delete_many_food_menus(selectedIds);
      // remove the menus from the store
      if (res) {
        remove_many_menus(selectedIds);
      }
        ...
        <div className="w-full h-full flex flex-col gap-4 justify-between">
          <DialogHeader>
            <DialogTitle className="mb-2">
              Delete All {selectedIds.length} Selected Menus
            </DialogTitle>
            <DialogDescription className="text-card-foreground">
              Please confirm that you want to delete these menus. This action is
              irreversible.
            </DialogDescription>
          </DialogHeader>
