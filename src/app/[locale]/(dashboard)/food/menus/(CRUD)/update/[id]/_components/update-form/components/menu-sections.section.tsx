...
import { UpdateFoodMenuValidationSchemaType } from "./update-menu-validation.schema";
...
import UpdateMenuSectionPopup from "../../../../../create/_components/create-form/components/update-menu-section-popup";
import CreateMenuSectionPopup from "../../../../../create/_components/create-form/components/add-menu-section-popup";

// Update menu sections section Main Info Section
...
const UpdateMenu_Sections_Section = forwardRef<HTMLDivElement, Props>(
  ({ id, formData }, ref) => {
    const {
      formState: { errors },
      control,
    } = useFormContext<UpdateFoodMenuValidationSchemaType>();
    ...
    const [toUpdateSection, setToUpdateSection] = useState<
      UpdateFoodMenuValidationSchemaType["sections"][0] | null
    >(null);

    // methods
    // handle add section
    const handleAddSection = useCallback(
      (sec: UpdateFoodMenuValidationSchemaType["sections"][0]) => {
        if (sec) {
    ...
    const handleUpdateSection = useCallback(
      (id: string, sec: UpdateFoodMenuValidationSchemaType["sections"][0]) => {
        if (id && sec) {
          sections_controller.field.onChange(
...
export default UpdateMenu_Sections_Section;
