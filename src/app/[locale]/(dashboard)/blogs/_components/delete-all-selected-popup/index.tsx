...
import { crud_delete_many_blogs } from "@/lib/curd/blog";
...
export default function DeleteAllSelectedBlogsPopup({
  selectedIds,
  unSelectedAll,
}: Props) {
  // rooms store
  const { remove_many_blogs } = useBlogTagsStore();
  ...
  const handleDelete = async () => {
    ...
    try {
      // delete many tag
      const res = await crud_delete_many_blogs(selectedIds);
      // remove the tag from the store
      if (res) {
        remove_many_blogs(selectedIds);
      }
      ...
      setOpen(false);
      // adding a toast
      toast.success("All Selected Blogs Where Deleted Successfully");
    } catch (err) {
        ...
        <div className="w-full h-full flex flex-col gap-4 justify-between">
          <DialogHeader>
            <DialogTitle className="mb-2">
              Delete All {selectedIds.length} Selected Blogs
            </DialogTitle>
            <DialogDescription className="text-card-foreground">
              Please be shure before you delete these blogs because this action
              can not be undone
            </DialogDescription>
          </DialogHeader>
