...
import { crud_delete_many_blog_tags } from "@/lib/curd/blog-tag";
...
export default function DeleteAllSelectedBlogTagsPopup({
  selectedIds,
  unSelectedAll,
}: Props) {
  // rooms store
  const { remove_many_tags } = useBlogTagsStore();
  ...
  const handleDelete = async () => {
    ...
    try {
      // delete many tag
      const res = await crud_delete_many_blog_tags(selectedIds);
      // remove the tag from the store
      if (res) {
        remove_many_tags(selectedIds);
      }
      ...
      setOpen(false);
      // adding a toast
      toast.success("All Selected Tags Where Deleted Successfully");
    } catch (err) {
        ...
        <div className="w-full h-full flex flex-col gap-4 justify-between">
          <DialogHeader>
            <DialogTitle className="mb-2">
              Delete All {selectedIds.length} Selected Tags
            </DialogTitle>
            <DialogDescription className="text-card-foreground">
              Once deleted, these tags will be remove the posts which use them.
            </DialogDescription>
          </DialogHeader>
