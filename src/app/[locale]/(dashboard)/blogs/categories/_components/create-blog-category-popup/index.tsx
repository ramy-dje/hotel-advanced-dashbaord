...
import {
  CreateBlogCategoryValidationSchemaType,
  CreateBlogCategoryValidationSchema,
} from "./create-blog-category.schema";
import { Label } from "@/components/ui/label";
import { Input } from "@/components/ui/input";
import InlineAlert from "@/components/ui/inline-alert";
import { crud_create_blog_category } from "@/lib/curd/blog-category";
...
export default function CreateBlogCategoryPopup() {
  ...
  const {
    ...
    reset,
    formState: { errors },
  } = useForm<CreateBlogCategoryValidationSchemaType>({
    resolver: zodResolver(CreateBlogCategoryValidationSchema),
  });
  const [open, setOpen] = useState(false);
  const [isLoading, setIsLoading] = useState(false);
  const [error, setError] = useState("");

  const handleCreate = async (data: CreateBlogCategoryValidationSchemaType) => {
    setIsLoading(true);
    setError("");
    try {
      const res = await crud_create_blog_category(data.name);
