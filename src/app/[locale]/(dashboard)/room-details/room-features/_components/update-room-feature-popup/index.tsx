...
import {
  UpdateRoomFeatureValidationSchema,
  UpdateRoomFeatureValidationSchemaType,
} from "./update-room-feature.schema";
...
import { crud_update_roomFeature } from "@/lib/curd/room-features";
...
interface Props {
  ...
  open: boolean;
  data: (UpdateRoomFeatureValidationSchemaType & { id: string }) | null;
}

export default function UpdateRoomFeaturePopup({ data, open, setOpen }: Props) {
  // room features store hook
  const { update_feature } = useRoomFeaturesStore();
  ...
  const {
    ...
    reset,
    formState: { errors },
  } = useForm<UpdateRoomFeatureValidationSchemaType>({
    resolver: zodResolver(UpdateRoomFeatureValidationSchema),
    defaultValues: {
      name: data?.name || "",
    },
  });
  ...
  const handleUpdate = async (
    newdata: UpdateRoomFeatureValidationSchemaType
  ) => {
    if (!data?.name || !data?.id) return;
    setIsLoading(true);
    setError("");
    try {
      const res = await crud_update_roomFeature(data.id, newdata);
      // update the feature in the store
      if (res) {
        update_feature(res.id, res);
      }
      setOpen(false);
      // resting the form
      reset({ name: "", icon: "" });
      // adding a toast
      toast.success("Feature Was Updated Successful");
    } catch (err) {
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      >
        ...
        <div className="w-full h-full">
          ...
          >
            <DialogHeader>
              <DialogTitle>Edit Room Feature</DialogTitle>
            </DialogHeader>
