...
import { RoomFeatures_TableColumns } from "./columns";
import {
...
import DeleteAllSelectedRoomFeaturesPopup from "./delete-all-selected-popup";

// The room features table and filters section

interface Props {
  meta?: Record<any, any>;
}

export default function RoomFeaturesTable({ meta }: Props) {
  // access
  const { has } = useAccess();
  // room features store hook
  const {
    features_list: data,
    features_number: total,
    set_many,
    set_total,
  } = useRoomFeaturesStore();

  const [searchState, setSearchState] = useState("");

  const { isFetching, totalPages, pagination, setPageRows, table, pageRows } =
    useServerTable<RoomFeatureInterface>({
      api: {
        url: "room-details/room-features",
      },
      column: RoomFeatures_TableColumns,
      data: data,
    ...
    <>
      ...
      <PageLayoutFilteringHeader>
        ...
        <div className="flex items-center gap-2">
          {/* the permission to delete many room features */}
          {has(["room_feature:delete"]) ? (
            <DeleteAllSelectedRoomFeaturesPopup
