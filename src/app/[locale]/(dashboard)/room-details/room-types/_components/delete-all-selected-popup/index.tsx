...
import { crud_delete_many_room_types } from "@/lib/curd/room-types";
...
export default function DeleteAllSelectedRoomTypesPopup({
  selectedIds,
  unSelectedAll,
}: Props) {
  // room types store hook
  const { remove_many_types } = useRoomTypesStore();
  ...
  const handleDelete = async () => {
    ...
    try {
      // delete many tag
      const res = await crud_delete_many_room_types(selectedIds);
      // remove the types from the store
      if (res) {
        remove_many_types(selectedIds);
      }
      ...
      setOpen(false);
      // adding a toast
      toast.success("All Selected Types Where Deleted Successfully");
    } catch (err) {
        ...
        <div className="w-full h-full flex flex-col gap-4 justify-between">
          <DialogHeader>
            <DialogTitle className="mb-2">
              Delete All {selectedIds.length} Selected Types
            </DialogTitle>
            <DialogDescription className="text-card-foreground">
              Please confirm that you want to delete these types. This action is
              irreversible.
            </DialogDescription>
          </DialogHeader>
