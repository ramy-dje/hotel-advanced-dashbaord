...
export default function DeleteAllCompletedSelectedReservationsPopup({
  selectedIds,
  unSelectedAll,
  changeStatus = false,
}: Props) {
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      // update all selected completed reservations to deleted status
      if (res) {
        ...
        if (changeStatus) {
          ...
          // remove all the reservations from the store
      ...
      setOpen(false);
      // adding a toast
      toast.success("Selected Reservations Moved All To Trash Successfully");
    } catch (err) {
        ...
        <div className="w-full h-full flex flex-col gap-4 justify-between">
          <DialogHeader>
            ...
            <DialogDescription className="text-card-foreground">
              Please before you archive all {selectedIds.length} selected
              completed reservations, be shure because this action can't be
              undone
            </DialogDescription>
          </DialogHeader>
