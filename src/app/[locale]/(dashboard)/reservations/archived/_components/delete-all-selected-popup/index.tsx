...
import { crud_delete_many_reservations } from "@/lib/curd/reservations";
...
export default function DeleteAllDeletedSelectedReservationsPopup({
  selectedIds,
  unSelectedAll,
}: Props) {
  // reservations store hook
  const { remove_many_reservations } = useReservationsStore();
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      const res = await crud_delete_many_reservations(selectedIds);
      if (res) {
      ...
      setOpen(false);
      // adding a toast
      toast.success("All Selected Reservations Where Deleted Successfully");
    } catch (err) {
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogTrigger asChild>
        ...
        >
          <HiOutlineTrash className="size-4" /> Delete all selected{" "}
          {selectedIds.length}{" "}
        </Button>
      </DialogTrigger>
      <DialogContent
        ...
        <div className="w-full h-full flex flex-col gap-4 justify-between">
          <DialogHeader>
            ...
            <DialogDescription className="text-card-foreground">
              Please before you delete all the {selectedIds.length} selected
              reservations, be shure because this action can't be undone
            </DialogDescription>
          </DialogHeader>
