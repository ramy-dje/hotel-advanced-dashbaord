...
import {
  ...
  UpdateReservationValidationSchemaType,
...
import CreateReservationForm from "./_components/update-form";
...
export default function UpdateReservationPage({
  ...
  const [cancel_loading, setCancel_loading] = useState(false);

  // form
  const methods = useForm<UpdateReservationValidationSchemaType>({
    resolver: zodResolver(UpdateReservationValidationSchema),
    // default values
    defaultValues: {
      rooms_number: 1,
      capacity_adults: 1,
  ...
  // handle save
  const handleUpdate = async (type: "approve" | "save" | "cancel") => {
    ...
    if (type == "approve") {
      ...
      setCancel_loading(true);
    }

    // create logic
    const data: UpdateReservationValidationSchemaType = methods.getValues();
    ...
    try {
      ...
      if (type == "approve") {
        ...
        // change the status
        await crud_update_reservation_status(reservation.id, "approved");

        // if the keep is true just change the status
        if (keep) {
          // change the reservation status
          change_many_reservations_status([reservation.id], "approved");
        } else {
          // remove the reservation from the store
          remove_reservation(reservation.id);
        }
      } else if (type == "cancel") {
        // change the status
        await crud_update_reservation_status(reservation.id, "canceled");

        // if the keep is true just change the status
        if (keep) {
          // change the reservation status
          change_many_reservations_status([reservation.id], "canceled");
        } else {
          // remove the reservation from the store
          remove_reservation(reservation.id);
        }
  ...
  // set the disabled
  useEffect(() => {
    setIsLoading(save_loading || save_and_approve_loading || fetchingRoomData);
    // set the disable form state manuel
    methods.control._disableForm(
      fetchingRoomData ||
        save_loading ||
        save_and_approve_loading ||
        cancel_loading
    );
  }, [
    save_and_approve_loading,
      ...
      {firstDataFetchLoading ? (
        ...
        <div className="w-full flex row-span-1 items-center justify-between gap-2 bg-background pt-4 lg:py-2 border-t lg:border-none">
          ...
          <div className="flex items-center gap-2">
            ...
            {/* approve && cancel button need permissions to be rendered  */}
            {has(["reservation-status:update"]) ? (
              <>
                <Button
                  disabled={methods.formState.disabled}
                  isLoading={cancel_loading}
