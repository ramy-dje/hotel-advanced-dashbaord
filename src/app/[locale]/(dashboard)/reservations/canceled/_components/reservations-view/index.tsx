...
export default function ViewCompletedReservationPage({
  ...
  const [unarchive_loading, setUnarchive_loading] = useState(false);
  ...
  // handle save
  const handleMoveToTrash = async (type: "trash" | "unarchive") => {
    if (!selectedReservation) return;
    try {
      if (type == "trash") {
        // change the status
        setTrash_loading(true);
        await crud_update_reservation_status(selectedReservation.id, "deleted");

        // if the keep is true just change the status
        if (keep) {
          // change the reservation status
          change_many_reservations_status([selectedReservation.id], "deleted");
        } else {
        ...
        setUnarchive_loading(true);
        await crud_update_reservation_status(selectedReservation.id, "pending");

        // if the keep is true just change the status
        if (keep) {
          // change the reservation status
          change_many_reservations_status([selectedReservation.id], "pending");
        } else {
          // remove the reservation from the store
          remove_reservation(selectedReservation.id);
        }
        ...
        // success
        toast.success("Reservations Changed to Pending Successfully");
      }

      setTrash_loading(false);
      setUnarchive_loading(false);
      setOpen(false);
      return;
    } catch (err) {
  ...
  // set the disabled
  useEffect(() => {
    setIsLoading(trash_loading || unarchive_loading);
    // set the disable form state manuel
    methods.control._disableForm(trash_loading || unarchive_loading);
  }, [trash_loading, unarchive_loading]);
          ...
          <div className="flex items-center gap-2">
            ...
            {has(["reservation-status:update"]) ? (
              <>
                ...
                <Button
                  disabled={methods.formState.disabled}
                  isLoading={unarchive_loading}
                  onClick={() => handleMoveToTrash("unarchive")}
                  type="button"
                  variant="outline"
                  size="sm"
                >
