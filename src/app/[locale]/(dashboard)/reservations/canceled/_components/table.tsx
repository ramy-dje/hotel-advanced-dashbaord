...
import { ArchivedReservations_TableColumns } from "./columns";
import {
...
import DeleteAllCanceledReservationsPopup from "./delete-all-canceled-popup";
import DeleteAllSelectedReservationsPopup from "./delete-all-selected-popup";
import useAccess from "@/hooks/use-access";
import useReservationsStore from "../../store";
import RecoverAllSelectedReservationsPopup from "./recover-all-selected-popup";

// The canceled reservations table and filters section

interface Props {
  meta?: Record<any, any>;
}

export default function ArchivedReservationsTable({ meta }: Props) {
  // access info
  const { has } = useAccess();
  ...
  const { isFetching, totalPages, pagination, setPageRows, table, pageRows } =
    useServerTable<ReservationsInterface>({
      ...
      column: ArchivedReservations_TableColumns,
      data: data,
      totalData: total,
        ...
        <div className="flex items-center gap-2">
          ...
          {has(["reservation-status:update"]) ? (
            <>
              ...
              <DeleteAllCanceledReservationsPopup disabled={data.length == 0} />
            </>
          ) : null}
          {/* filter */}
          {/* <Button
