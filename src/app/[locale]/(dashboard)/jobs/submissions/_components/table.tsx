...
import DeleteAllSelectedJobSubmissionsPopup from "./delete-all-selected-popup";
import JobSubmissionsPageFiltering from "./filtering";
...
export default function JobSubmissionsTable({ meta }: Props) {
  // access
  const { has } = useAccess();
  // job submissions store hook
  const {
    submissions_list: data,
    submissions_number: total,
    set_many,
    set_total,
  } = useJobSubmissionsStore();
  ...
  const {
    ...
    setPageRows,
    table,
  } = useServerTable<JobSubmissionInterface>({
    api: {
      url: "jobs/submissions",
    },
    column: JobSubmissions_TableColumns,
    data: data,
    ...
    <>
      ...
      <PageLayoutFilteringHeader>
        ...
        <div className="flex items-center max-sm:flex-row gap-2">
          {/* the permission to delete many submissions */}
          {has(["job_position:delete"]) ? (
            <DeleteAllSelectedJobSubmissionsPopup
