...
import { Roles_TableColumns } from "./columns";
import {
...
import DeleteAllSelectedRolesPopup from "./delete-all-selected-popup";

// The roles table and filters section

interface Props {
  meta?: Record<any, any>;
}

export default function RolesTable({ meta }: Props) {
  // access
  const { has } = useAccess();
  // roles store hook
  const {
  ...
  const { isFetching, totalPages, pagination, setPageRows, table, pageRows } =
    useServerTable<RoleInterface>({
      api: {
        url: "roles",
      },
      column: Roles_TableColumns,
      data: data,
