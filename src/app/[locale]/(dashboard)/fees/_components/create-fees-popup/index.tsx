...
import {
  CreateFeesValidationSchemaType,
  CreateFeesValidationSchema,
} from "./create-fees.schema";
import { Label } from "@/components/ui/label";
import { Input } from "@/components/ui/input";
import InlineAlert from "@/components/ui/inline-alert";
import { crud_create_fee } from "@/lib/curd/fees";
...
export default function CreateFeePopup() {
  const { add_fee } = useFeesStore();
  ...
  const {
    ...
    watch,
    formState: { errors },
  } = useForm<CreateFeesValidationSchemaType>({
    resolver: zodResolver(CreateFeesValidationSchema),
  });
  const [open, setOpen] = useState(false);
  const [isLoading, setIsLoading] = useState(false);
  const [error, setError] = useState("");

  const handleCreate = async (data: CreateFeesValidationSchemaType) => {
    setIsLoading(true);
    setError("");
    try {
      const res = await crud_create_fee(data.name, data.type, data.amount);
      ...
      reset({ name: "" });
      // adding a toast
      toast.success("Fee created Successful");
    } catch (err) {
      if (err == 409) {
    ...
    >
      <DialogTrigger asChild>
        <Button className="gap-2 font-normal w-1/2 md:w-auto">
          <HiOutlinePlus className="size-4" /> Add Fee
        </Button>
      </DialogTrigger>
      <DialogContent
