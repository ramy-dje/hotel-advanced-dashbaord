...
import { crud_delete_many_fees } from "@/lib/curd/fees";
...
export default function DeleteAllSelectedTaxesPopup({
  ...
  // rooms store
  const { remove_many_fees } = useFeesStore();
  ...
  const handleDelete = async () => {
    ...
    try {
      // delete many taxes
      const res = await crud_delete_many_fees(selectedIds);
      // remove the taxes from the store
      if (res) {
        remove_many_fees(selectedIds);
      }
      ...
      setOpen(false);
      // adding a toast
      toast.success("All Selected Fees Where Deleted Successfully");
    } catch (err) {
        ...
        <div className="w-full h-full flex flex-col gap-4 justify-between">
          <DialogHeader>
            <DialogTitle className="mb-2">
              Delete All {selectedIds.length} Selected Fees
            </DialogTitle>
            <DialogDescription className="text-card-foreground">
              Once deleted, these fees will be remove the posts which use
              them.
            </DialogDescription>
          </DialogHeader>
