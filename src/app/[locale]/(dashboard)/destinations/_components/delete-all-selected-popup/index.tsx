...
import { crud_delete_many_destinations } from "@/lib/curd/destination";
...
export default function DeleteAllSelectedDestinationsPopup({
  selectedIds,
  unSelectedAll,
}: Props) {
  // rooms store
  const { remove_many_destinations } = useDestinationsStore();
  ...
  const handleDelete = async () => {
    ...
    try {
      // delete many tag
      const res = await crud_delete_many_destinations(selectedIds);
      // remove the tag from the store
      if (res) {
        remove_many_destinations(selectedIds);
      }
      ...
      setOpen(false);
      // adding a toast
      toast.success("All Selected Destinations Where Deleted Successfully");
    } catch (err) {
        ...
        <div className="w-full h-full flex flex-col gap-4 justify-between">
          <DialogHeader>
            <DialogTitle className="mb-2">
              Delete All {selectedIds.length} Selected Destinations
            </DialogTitle>
            <DialogDescription className="text-card-foreground">
              Please be shure before you delete these destinations because this
